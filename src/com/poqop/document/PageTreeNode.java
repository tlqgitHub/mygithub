...
class PageTreeNode {
	 ...
	 * 
	 * @param documentView
	 * @param localPageSliceBounds
	 * @param page
	 * @param treeNodeDepthLevel
	 * @param parent
	 */
	PageTreeNode(DocumentView documentView, RectF localPageSliceBounds,
			Page page, int treeNodeDepthLevel, PageTreeNode parent) {
		this.documentView = documentView;
		this.pageSliceBounds = evaluatePageSliceBounds(localPageSliceBounds,
				parent);
		this.page = page;
	...
	void draw(Canvas canvas) {
		if (getBitmap() != null) {
			canvas.drawBitmap(getBitmap(), new Rect(0, 0, getBitmap()
					.getWidth(), getBitmap().getHeight()), getTargetRect(),
					bitmapPaint);
		}
		if (children == null) {
			return;
	...
	private void invalidateChildren() {
		if (thresholdHit() && children == null && isVisible()) {
			...
			children = new PageTreeNode[] {
					new PageTreeNode(documentView, new RectF(0, 0, 0.5f, 0.5f),
							page, newThreshold, this),
					new PageTreeNode(documentView, new RectF(0.5f, 0, 1.0f,
							0.5f), page, newThreshold, this),
					new PageTreeNode(documentView, new RectF(0, 0.5f, 0.5f,
							1.0f), page, newThreshold, this),
					new PageTreeNode(documentView, new RectF(0.5f, 0.5f, 1.0f,
							1.0f), page, newThreshold, this) };
		}
	...
	private boolean thresholdHit() {
		...
		return (mainWidth * zoom * height)
				/ (treeNodeDepthLevel * treeNodeDepthLevel) > SLICE_SIZE;
	}
	...
	private void decodePageTreeNode() {
		...
		setDecodingNow(true);
		documentView.decodeService.decodePage(this, page.index,
					...
					public void decodeComplete(final Bitmap bitmap) {
						documentView.post(new Runnable() {
							public void run() {
								...
								page.setAspectRatio(documentView.decodeService
										.getPageWidth(page.index),
										documentView.decodeService
												.getPageHeight(page.index));
								invalidateChildren();
							}
						});
					}
	...
	private RectF evaluatePageSliceBounds(RectF localPageSliceBounds,
			PageTreeNode parent) {
		if (parent == null) {
			return localPageSliceBounds;
		}
		final Matrix matrix = new Matrix();
		matrix.postScale(parent.pageSliceBounds.width(),
				parent.pageSliceBounds.height());
		matrix.postTranslate(parent.pageSliceBounds.left,
				parent.pageSliceBounds.top);
	...
	private Rect getTargetRect() {
		if (targetRect == null) {
			...
			targetRect = new Rect((int) targetRectF.left,
					(int) targetRectF.top, (int) targetRectF.right,
