...
public class PdfPage implements CodecPage {
	...
	public Bitmap renderBitmap(int width, int height, RectF pageSliceBounds) {
		...
		matrix.postScale(width / getMediaBox().width(), -height
				/ getMediaBox().height());
		matrix.postTranslate(0, height);
		matrix.postTranslate(-pageSliceBounds.left * width,
				-pageSliceBounds.top * height);
		matrix.postScale(1 / pageSliceBounds.width(),
				1 / pageSliceBounds.height());
	...
	public Bitmap render(Rect viewbox, Matrix matrix) {
		...
		return Bitmap.createBitmap(bufferarray, width, height,
				Bitmap.Config.RGB_565);
		/*
		 * ByteBuffer buffer = ByteBuffer.allocateDirect(width * height * 2);
		 * render(docHandle, docHandle, mRect, matrixArray, buffer,
		 * ByteBuffer.allocateDirect(width * height * 8)); final Bitmap bitmap =
		 * Bitmap.createBitmap(width, height, Bitmap.Config.RGB_565);
		 * bitmap.copyPixelsFromBuffer(buffer); return bitmap;
		 */
	}
	...
	private static native void render(long dochandle, long pagehandle,
			int[] viewboxarray, float[] matrixarray, ByteBuffer byteBuffer,
			ByteBuffer tempBuffer);

	private native void nativeCreateView(long dochandle, long pagehandle,
			int[] viewboxarray, float[] matrixarray, int[] bufferarray);
}
