...
public class XMLRPCSerializer {
	...
	static void serialize(XmlSerializer serializer, Object object)
		...
		if (object instanceof Integer || object instanceof Short
			...
			serializer.startTag(null, TYPE_INT).text(object.toString())
					.endTag(null, TYPE_INT);
	...
	public static Object deserialize(XmlPullParser parser)
		...
		if (typeNodeName.equals(TYPE_INT) || typeNodeName.equals(TYPE_I4)) {
			...
			// System.out.println("这个member是"+parser.getName());
			while (parser.getName().equals(TAG_MEMBER)) {
				...
				while (true) {
					...
					// System.out.println("这个name是"+name);
					if (name.equals(TAG_NAME)) {
						memberName = parser.nextText();
						// System.out.println("卢军"+memberName);
					} else if (name.equals(TAG_VALUE)) {
						memberValue = deserialize(parser);
						// System.out.println("卢蛋"+memberValue);
					} else {
						break;
					}
				}
				if (memberName != null && memberValue != null) {
					// System.out.println("键   "+memberName);
					// System.out.println("值  "+memberValue);
